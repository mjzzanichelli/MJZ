var $$ = (
    function (_$$) {
        ...
        var _public = {
            ...
            , removeEvent: _private.removeEvent

            , triggerEvent: _private.triggerEvent
            ...
            , removeAttributes: function (element, params) {
                var attr;
                for (attr in params) if (params.hasOwnProperty(attr)) element.removeAttribute(attr);
                attr = null;
            }
	        ...
	        , isParent: function (element, parent) {
	            if (element != undefined && parent != undefined) {
	                if (element == parent) return true;
	                    ...
	                    if (element.parentNode!=undefined && element.parentNode != document) return _public.isParent(element.parentNode, parent);
	                    else return false;
	                }
	            } else return false;
	        }
			...
			, addJS: function (params, callback) {
			    if (params && params["path"]) {
			        ...
			        var _callback = function () {
			            var _callback = params["callback"] || callback || function () { };
			            _callback();
			            params["callback"] = callback = undefined;
			        }
			        _script.onreadystatechange = function () {
			            if (this.readyState == 'complete') _callback();
			        };
